...
class PdfDownloader:
	...
	def check_save_folder(self):
		...
		if not os.path.exists(fpath):
			try:
				...
				ret=False
				print(f'[check_save_folder][exception]:folder({fpath})')
				print(f'[check_save_folder][exception]:ex({ex})')
				traceback.print_exc()
	...
	def parse_request_url(self,url):
		...
		try:
			...
			print(f'[parse_request_url][exception]:url({url})')
			print(f'[parse_request_url][exception]:ex({ex})')
			traceback.print_exc()
			ret=False
			items=None
		...
		if ret:
			pid=items.get('pid')
			if pid is None:
				print(f'[parse_request_url][error]:no (pid) in url.')
				ret=False
			else:
	...
	def request_pdf_url(self,url):
		'''
		'''
		self.my_log(f'[request_pdf_url]:url({url})')
		...
		try:
			...
			print(f'[request_pdf_url][exception]:url({url}) ex({ex})')
			traceback.print_exc()
			ret=False
		...
		if ret:
			self.my_log(f'[request_pdf_url]:req-type:({type(req)})')
			try:
				soup=BeautifulSoup(req.content,'html.parser')
				with open('./find_pdf.htm','w') as fp:
					fp.write(f'{soup}')
					
			except Exception as ex:
				print(f'[request_pdf_url][exception]:({ex})')
				traceback.print_exc()
				soup=None
				ret=False
		...
		if ret:
			...
			self.my_log(f'[request_pdf_url]:soup-type:({type(soup)})')
			...
			if script is None:
				...
				if rets is not None:
					...
					print(f'[request_pdf_url][error]:parse location FAILED.')
					ret=False
	...
	def download_pdf(self,url):
		'''
		'''
		self.my_log(f'[download_pdf]:url({url}).')
		...
		try:
			...
			print(f'[download_pdf][exception]:url({url})')
			print(f'[download_pdf][exception]:ex({ex})')
			traceback.print_exc()
			ret=False

		save_file=f'{self.save_folder}/{self.pdf_file_name}'
		if ret:
			self.my_log(f'[download_pdf]:save to ({save_file})')
			try:
				with open(save_file,'wb') as f:
					f.write(req.content)
			except Exception as ex:
				print(f'[download_pdf][exception]:save_file({save_file})')
				print(f'[download_pdf][exception]:({ex})')
				traceback.print_exc()
				ret=False

		self.my_log(f'[download_pdf]:done.ret({ret})')
		return ret
